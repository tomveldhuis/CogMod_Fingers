...
struct BotModel_Thijs : BotModelProtocol {
        ...
        while !done {
            ...
            switch (goal.slotvals["state"]!.description) {
                ...
                case "waiting":
                    ...
                    // Add result to DM
                    let newResult = Chunk(s: "lastResult", m: model)
                    ...
                    addToTrace(string: "Added result to DM")
    ...
    func checkExtremes(goal: Chunk) {
        ...
        if let retrievedChunk = result {
            // Check if the retrieved result is an extreme: either 0 or playerCount
            let lastResult = Int(retrievedChunk.slotvals["result"]!.number()!)
            if lastResult == 0 {
                // Check if player wants to break away
                
            } else if lastResult == Int(goal.slotvals["numPlayers"]!.number()!) {
            ...
            // Retrieval failure
            checkPersonalPattern(goal: goal)
        }
    }
    
    func breakTendencyCheck() -> Bool {
    ...
    func checkPersonalPattern(goal: Chunk) {
